...
# dsd = np.ndarray(shape=([25000, 200]))
# for x in x_train:
#     print(x)
...
# 从每个句子向量的后面截取200个，不足的用0做padding
...
"""
# 原句子向量的长度和200，哪个小记录哪个
x_len_train = np.array([min(len(x), sentence_size) for x in x_train_variable])
x_len_test = np.array([min(len(x), sentence_size) for x in x_test_variable])

# (x, length, y)对应(x_train, x_len_train, y_train)
def parser(x, length, y):
    # 把整个200列数据起名叫做"x", 后面一列叫做"len",并且把"x"和"len"统一归为features
    features = {"x": x, "len": length}
    return features, y

def train_input_fn():
    # (25000, 200) (25000, 1) (25000, 1)拼接在一起
...
def train_and_evaluate(classifier):
    ...
    classifier.train(input_fn=train_input_fn, steps=25000, hooks=HOOKS)

...
def processdata():
    ...
    model = LogisticRegression(class_weight="balanced")###样本失衡，正例样本远比负例样本少，每个分类的权重与该分类在样品中出现的频率成反比。
    ###sklearn里面逻辑回归默认是用L2正则化项的，我这个模型里面特征不多，就4维，所以不需要L1正则化，只是为了防止过拟合，L2正则化就够了
